...
- (instancetype)initWithQrType:(MMScanType)type priceStr:(NSString *)price onFinish:(void (^)(NSString *result, NSError *error))finish {
    self = [super init];
    if (self) {
        self.scanType = type;
...
- (void)drawTitle
{
    if (!_tipTitle)
    {
        ...
        _tipTitle.text = @"请将取景框对准二维码,即可自动扫描";
        _tipTitle.textColor = [UIColor whiteColor];
        [self.view addSubview:_tipTitle];
        
        
        self.priceTitle = [[UILabel alloc]init];
        _priceTitle.bounds = CGRectMake(0, 0, 300, 50);
        _priceTitle.center = CGPointMake(CGRectGetWidth(self.view.frame)/2, self.view.center.y + self.view.frame.size.width/2);
        _priceTitle.font = [UIFont systemFontOfSize:13];
        _priceTitle.textAlignment = NSTextAlignmentCenter;
        _priceTitle.numberOfLines = 0;
        _priceTitle.text = [NSString stringWithFormat:@"收款金额¥ %@",self.pricestr];
        _priceTitle.textColor = ThemeColor;
        [self.view addSubview:_priceTitle];
        
        
        
    }
    _priceTitle.layer.zPosition = 1;
    [self.view bringSubviewToFront:_priceTitle];
...
- (void)setNavItem:(MMScanType)type {
    if(type == MMScanTypeBarCode) {
        ...
        UIEdgeInsets inset = UIEdgeInsetsMake(0, 0, 0, 0);
         ...
         [rightButton setTitle:@"打开闪光灯" forState:UIControlStateNormal];
         [rightButton setTitle:@"关闭闪光灯" forState:UIControlStateSelected];
        ...
        [rightButton addTarget:self action:@selector(light_buttonAction:) forControlEvents:UIControlEventTouchUpInside];
