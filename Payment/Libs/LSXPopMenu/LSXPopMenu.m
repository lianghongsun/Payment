...
@property(assign,nonatomic)CGFloat newcellHeight;
...
-(void)setModel:(LSXPopMenuModel *)Model{
    ...
    if (Model.imgName) {
        ...
        CGFloat imgY=(_newcellHeight-Model.imgH)/2;
        _imgView.frame=CGRectMake(imgX, imgY, Model.imgW, Model.imgH);
        _titLa.frame=CGRectMake(CGRectGetMaxX(_imgView.frame)+10, 0, W-10-CGRectGetMaxX(_imgView.frame), _newcellHeight);
        _imgView.image=[UIImage imageNamed:Model.imgName];
    }else {
        [_imgView setHighlighted:YES];
        _titLa.textAlignment=NSTextAlignmentCenter;
        _titLa.frame=CGRectMake(0, 0, W, _newcellHeight);
    }
